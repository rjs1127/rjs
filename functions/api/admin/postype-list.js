  ...
  try {
    ...
    for (let index = 1; index < values.length; index += 1) {
      ...
      items.push({
        ...
        publishType: cell(row, headerIndex, "publishType") || (/\/series\/\d+/i.test(cell(row, headerIndex, "url")) || cell(row, headerIndex, "linkType") === "manual" ? "다회차" : "단일글"),
        linkType: cell(row, headerIndex, "linkType") || (/\/series\/\d+/i.test(cell(row, headerIndex, "url")) ? "series" : "post"),
